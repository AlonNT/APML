...
class DuelQFunction(keras.Model):
    ...
    def __init__(self):
        ...
        self.advantage_affine1 = keras.layers.Dense(units=128, activation=tf.nn.relu)
        self.advantage_affine2 = keras.layers.Dense(units=N_ACTIONS)
        self.value_affine1 = keras.layers.Dense(units=128, activation=tf.nn.relu)
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        q = self.add_value_advantage([a, v])

        return q


class DuelQN(bp.Policy):
    """
    ...
    def cast_string_args(self, policy_args):
        ...
        for param_name in DuelQN.params.keys():
            default_value = DuelQN.params[param_name]
    ...
    def init_run(self):
        ...
        self.q_function = DuelQFunction()
        self.target_function = DuelQFunction()
    ...
    def act(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        if round == self.game_duration - 1:
            ...
            with open(os.path.join(out_dir, '{}_params.json'.format(s)), 'w') as f:
                json.dump({k: self.__dict__[k] for k in self.__dict__.keys() if k in DuelQN.params.keys()}, f)
        # TODO --------------------------------------------------
